...
enum ZegoMediaOption { autoPlayAudio, autoPlayVideo, publishLocalAudio, publishLocalVideo }
...
class ZegoExpressManager {
  ...
  void Function(ZegoUpdateType updateType, List<String> userIDList, String roomID)? onRoomUserUpdate;

  /// Trigger when device's status of user with [userID] has been update
  void Function(ZegoDeviceUpdateType updateType, String userID, String roomID)? onRoomUserDeviceUpdate;
  ...
  void Function(List<ZegoRoomExtraInfo> roomExtraInfoList)? onRoomExtraInfoUpdate;
  ...
  String _roomID = "";
  ZegoMediaOptions _mediaOptions = [ZegoMediaOption.autoPlayAudio, ZegoMediaOption.autoPlayVideo];
  ...
  void createEngine(int appID, String appSign) {
    ...
    ZegoExpressEngine.onRoomUserUpdate = (String roomID, ZegoUpdateType updateType, List<ZegoUser> userList) {
      ...
      if (updateType == ZegoUpdateType.Add) {
        for (final user in userList) {
          ...
          ZegoParticipant participant = ZegoParticipant(user.userID, user.userName);
    ...
    ZegoExpressEngine.onRemoteCameraStateUpdate = (String streamID, ZegoRemoteDeviceState state) {
      if (_streamDic.containsKey(streamID)) {
        ...
        var type =
            state == ZegoRemoteDeviceState.Open ? ZegoDeviceUpdateType.cameraOpen : ZegoDeviceUpdateType.cameraClose;
    ...
    ZegoExpressEngine.onRemoteMicStateUpdate = (String streamID, ZegoRemoteDeviceState state) {
      if (_streamDic.containsKey(streamID)) {
        ...
        var type = state == ZegoRemoteDeviceState.Open ? ZegoDeviceUpdateType.micUnMute : ZegoDeviceUpdateType.micMute;
        if (onRoomUserDeviceUpdate != null) {
          onRoomUserDeviceUpdate!(type, participant.userID, _roomID);
        }
      }
    ...
    ZegoExpressEngine.onNetworkQuality =
        (String userID, ZegoStreamQualityLevel upstreamQuality, ZegoStreamQualityLevel downstreamQuality) {
  ...
  Future<void> joinRoom(String roomID, ZegoUser user, ZegoMediaOptions options) async {
  ...
  Widget? getRemoteVideoView(String userID) {
    ...
    if (_participantDic[userID]?.viewID != -1) {
      ZegoExpressEngine.instance.destroyPlatformView(_participantDic[userID]!.viewID);
    }
  ...
  void enableCamera(bool enable) {
    if (enable && !_isPlayingStream) {
      ZegoExpressEngine.instance.startPublishingStream(_localParticipant.streamID);
  ...
  Future<int> setRoomExtraInfo(String key, String value) async {
    ...
    var result = await ZegoExpressEngine.instance.setRoomExtraInfo(_roomID, key, value);
